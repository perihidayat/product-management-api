...
class ProductDetailController extends Controller {
  ...
  def index = Authenticated(
    rs => {
      val result = Await.result(db.run(productDetails.result), Duration.Inf)
      Ok(toJson(Json.obj("productDetails" -> result)))
    })

  def insertOrUpdate(): Action[AnyContent] = Authenticated(
    rs => {
      parseBodyToProductDetail(rs) match {
        case Some(x) => {
          val result = Await.result(db.run(productDetails.insertOrUpdate(x)), Duration.Inf)
  ...
  def delete(id: Option[Int]): Action[AnyContent] = Authenticated(
    rs => {
      val result = Await.result(db.run(productDetails.filter { _.id === id }.delete), Duration.Inf)
