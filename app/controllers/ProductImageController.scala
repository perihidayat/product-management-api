...
class ProductImageController extends Controller {
  ...
  def index = Authenticated(
    rs => {
      val result = Await.result(db.run(productImages.result), Duration.Inf)
      Ok(toJson(Json.obj("productImages" -> result)))
    })

  def insertOrUpdate(): Action[AnyContent] = Authenticated(
    rs => {
      parseBodyToProductImage(rs) match {
        case Some(x) => {
          val result = Await.result(db.run(productImages.insertOrUpdate(x)), Duration.Inf)
  ...
  def delete(id: Option[Int]): Action[AnyContent] = Authenticated(
    rs => {
      val result = Await.result(db.run(productImages.filter { _.id === id }.delete), Duration.Inf)
